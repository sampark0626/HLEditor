...
9. **자동 검출 기준 UI 노출** (미구현) — 오디오 후보 검출 민감도(`SPIKE_PERCENTILE`/`SPIKE_MIN_DB`)를 UI에서 조절. dB·percentile 용어는 숨기고 **3단 프리셋 버튼**으로 노출하기로 확정:
   - **많이 잡기** (놓치기 싫을 때, 후보↑·비전비용↑) ≈ percentile 90 / 6dB
   - **보통** (기본, 균형) ≈ 95 / 8dB
   - **엄선** (확실한 장면만, 후보↓·저렴) ≈ 98 / 11dB
   원시 값은 '고급 설정'에 풀어쓴 라벨("최소 소리 크기 변화" 등 + 툴팁)로 숨김.
10. **단계별 진행 표시(프로그레스 바)** (미구현) — 진행 로그에 자동검출/비전판별/영상생성 각각의 진행률을 시각적으로 표시해 대기 체감을 줄임.
    - 비전판별: "32/72 완료" 식 카운트 + 진행 바 (현재 서버는 후보별 완료를 알지만 UI는 끝나야 한 번에 받음 → SSE/폴링으로 실시간 스트리밍 필요)
    - 영상생성: "클립 18/53 인코딩 중" 식 단계 표시
    - 자동검출: 짧으므로 스피너 수준으로 충분
11. **stream-copy 분기** — 속도 우선 모드 옵션화 (미구현)
12. **BAND 업로드 자동화** — 다운로드→편집→업로드 한 사이클 에이전트화 (검토 완료, 세부 설계 확정). BAND는 영상 직접 업로드 API가 없으므로 **YouTube 경유**로 우회:
    - **YouTube 업로드**: `videos.insert`(재개형), 일부공개(unlisted). OAuth2 + 리프레시 토큰. 업로드 1건 ≈ 1,600 units / 일 10,000 → 하루 ~6건.
    - **자동 글 생성**: `results.json` 비전 결과로 타임스탬프 장면 목록 작성. 영상 내 시각(`i×13+8`초)과 원본 경기 시각을 함께, **YouTube 챕터 형식**(0:00 시작)으로 → 유튜브가 챕터 자동 생성. 유형 한글 매핑(goal→골 등).
    - **썸네일**: `highlight=true` 중 confidence 최대 후보의 **peak 시점을 원본에서 고해상도 1프레임 재추출** → `thumbnails.set`. 단 커스텀 썸네일은 **채널 전화 인증 필요**(미인증 시 자동 썸네일 사용).
    - **BAND 게시**: `Write Post`로 글 본문 + YouTube 링크 + `do_push`. (본문 글자수/링크 카드 동작은 공식 문서 최종 확인 필요)
    - **다운로드 레그**: 원본 영상 출처(XbotGo/클라우드)의 API 제공 여부에 의존 — 미확정.

### 보류 — 향후 성능 향상 시 검토할 아이디어 (현재 작업 대상 아님)
- **STT 보조 신호** — 현장음에서 "골!"·"슛!" 등 외침을 Speech-to-Text로 인식해 볼륨·비전이 애매한 후보의 점수를 보강하는 3차 신호. 동호회 경기는 해설 없고 웅성거림 위주라 인식률이 낮아 효과 불확실. 현 성능으로는 불필요하다고 판단해 **다음 업데이트 대상에서 제외**. 추후 정확도를 더 끌어올려야 할 때 재검토.
