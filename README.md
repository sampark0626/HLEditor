...
[영상 추가]
   │
   ├─ 수동 모드 ──▶ 오디오 분석 → 팬 궤적 분석 → Gemini 비전 판별 → 리뷰 → 영상 생성 → YouTube 업로드 → BAND 게시
   │
   └─ 원스톱 ────▶ 오디오 분석 → 팬 궤적 분석 → Gemini 비전 판별 → 자동 승인 → 영상 생성 → YouTube 업로드 [→ BAND 게시]
                                                                          (버튼 한 번으로 전체 자동)
```

- **1차 (오디오)**: 골·환호 등 볼륨 급증 시점을 rolling baseline 대비 상대 상승폭으로 검출. 빠르고 무료.
- **2차 (비전)**: 후보 구간 프레임을 Gemini에 전송해 유형·신뢰도 판별. 정확하지만 유료.
- **3차 (팬 궤적)**: XbotGo 카메라의 좌우 팬 움직임을 로컬에서 분석해 "골문 앞 정지"·"빠른 전개" 상태의 후보에 신뢰도를 +0.10 보정(승격 전용). 무료·로컬 계산이며 실패해도 파이프라인은 계속 진행.
...
3. 영상별로 펼쳐 후보 목록 확인 (썸네일·AI 설명·신뢰도·유형 포함). 신뢰도 옆 **▲ 표시**는 카메라 팬 신호(골문 앞 정지/빠른 전개)로 보정된 값 — 마우스를 올리면 근거가 표시됨
