...
1. **영상 선택 & 후보 검출** — `찾아보기…`로 파일을 고르고(또는 경로 입력) 오디오 급증 후보를 검출 (무료)
2. **Gemini 비전 판별** — 병렬 수를 정하고 실행. 결과가 표에 채택/확인필요/제외로 표시되고 `results.json`에 자동 저장. **■ 중단** 버튼으로 진행 중 취소 가능(이미 시작된 호출만 마무리). 끝나면 **사용 토큰·예상 비용(USD)** 이 표시됨
3. **신뢰도 임계 조정** — 슬라이더를 움직이면 **API 재호출 없이** 채택 구간이 실시간으로 바뀜
4. **하이라이트 영상 생성** — **제목(우상단)** 을 입력하면 영상 우상단에 작은 타이틀 워터마크가 입혀짐(비우면 없음). 출력 파일명을 정하고 생성하면 결과 영상을 바로 미리보기
